"""Schema and graph validation for benchmark scenario definitions."""
    ...
    for scenario in scenarios.values():
        if 'components' not in scenario:
            ...
            if (
                ...
                or any(not isinstance(name, str) or not name for name in validators)
                or len(validators) != len(set(validators))
            ):
                raise error_type(
                    f'Scenario {scenario["name"]}: validators must be a list of unique non-empty strings'
                )
        ...
        if 'validators' in scenario:
            raise error_type(
                f'Scenario {scenario["name"]}: validators belong on atomic scenarios, not compositions'
            )

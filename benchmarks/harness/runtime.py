...
def plan_function_call(identifier: str | None, component: PlanComponent) -> str | None:
    if identifier is None or component.players == 0 or component.first is None or component.last is None:
        return None
    arguments = {
...
def compile_active_plan(server: Path, plan: list[PlanComponent]):
    ...
    for component in plan:
        prepare_call = plan_function_call(component.measurement_prepare, component)
        if prepare_call:
            measurement_prepare.append(prepare_call)
    for component in plan:
        reset_call = plan_function_call(component.measurement_reset, component)
        if reset_call:
            measurement_reset.append(reset_call)
    ...
    for name, lines in (
        ...
        ('tick', tick),
        ('teardown', teardown),
        ('measurement_prepare', measurement_prepare),

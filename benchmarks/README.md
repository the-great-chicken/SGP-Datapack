...
- **GC pauses ms/tick**: JVM stop-the-world pause time during the capture (from `-Xlog:gc`), which `/perf` attributes to whatever section was running. **Heap after GC pause** is the occupancy right after the last pause (an upper bound on what is reachable, the pause may be a young collection) and **Heap floor** is the minimum of the profiler's per-tick heap samples: PackTest dummy players retain every packet sent to them and stay reachable after they leave (see the PackTest issues), so heavy scenarios (rays) retain memory for the whole session and later runs of a session become GC-bound. When more than half the heap is still in use after a run's last GC pause, the runner restarts the JVM before the next run (recorded in `metadata.json` and the summary); the GC ms/tick column shows what remains inside a run.

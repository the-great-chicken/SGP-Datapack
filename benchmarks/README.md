...
Atomic scenarios live in `benchmarks/scenarios/`. Compose them by referencing other scenarios in JSON:
...
Define `setup`, `tick`, `teardown`, integer parameters, and optional counters in `benchmarks/scenarios/<name>.json`; put the functions under `benchmarks/fixtures/data/sgp.bench/function/scenarios/<name>/`. Functions receive `first`, `last`, `players`, and scenario parameters.

...
$execute positioned $(x) $(y) $(z) unless entity @e[tag=sgp.ci.illusion_$(group),tag=sgp.direction_$(direction),distance=..0.01,type=mannequin] run function sgp.ci:illusions_movement/position_failure with storage sgp.ci:illusions_movement position_check

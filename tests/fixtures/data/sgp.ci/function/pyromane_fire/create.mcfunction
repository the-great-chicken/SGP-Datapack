...
# Reproduce the charge link established when the ability creates its interaction.
$execute positioned $(x) ~1 ~0.5 as @n[tag=sgp.ci.fire,distance=..0.1,type=tnt] run function #bs.id:give_suid
$execute positioned $(x) ~1 ~0.5 run scoreboard players operation @e[tag=sgp.ci.fire,distance=..0.1,type=interaction] bs.link.to = @n[tag=sgp.ci.fire,distance=..0.1,type=tnt] bs.id

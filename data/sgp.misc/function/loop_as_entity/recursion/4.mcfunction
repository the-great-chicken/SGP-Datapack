#> sgp.misc:loop_as_entity/recursion/4
# `{uuid: entity uuid, command: "run kill @s"}`
# Shard 4 of 8 (generated; all shards are identical apart from their successor). Elements
# 4, 12, ... of every list run through this file, so its macro cache only ever holds
...
function sgp.misc:loop_as_entity/recursion/5 with storage sgp:data temp.run_args

#> sgp.misc:loop_as_entity/recursion/5
# `{uuid: entity uuid, command: "run kill @s"}`
# Shard 5 of 8 (generated; all shards are identical apart from their successor). Elements
# 5, 13, ... of every list run through this file, so its macro cache only ever holds
...
function sgp.misc:loop_as_entity/recursion/6 with storage sgp:data temp.run_args

#> sgp.misc:loop_as_entity/recursion/2
# `{uuid: entity uuid, command: "run kill @s"}`
# Shard 2 of 8 (generated; all shards are identical apart from their successor). Elements
# 2, 10, ... of every list run through this file, so its macro cache only ever holds
...
function sgp.misc:loop_as_entity/recursion/3 with storage sgp:data temp.run_args

#> sgp.misc:loop_as_entity/recursion/6
# `{uuid: entity uuid, command: "run kill @s"}`
# Shard 6 of 8 (generated; all shards are identical apart from their successor). Elements
# 6, 14, ... of every list run through this file, so its macro cache only ever holds
...
function sgp.misc:loop_as_entity/recursion/7 with storage sgp:data temp.run_args

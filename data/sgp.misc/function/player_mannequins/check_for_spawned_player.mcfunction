...
$execute as @a[tag=sgp.has_mannequin] unless entity @s[dx=$(dx),dy=$(dy),dz=$(dz)] \
    run function sgp.misc:player_mannequins/disappear

# Detects if a player just entered bounds
$execute as @a[tag=sgp.in_game,tag=!sgp.has_mannequin,dx=$(dx),dy=$(dy),dz=$(dz)] \

...
execute store result storage sgp.ci:summon_multiple callback_context.called int 1 if entity @e[tag=sgp.test.summon_callback,scores={sgp.dummy=1},distance=..2,type=marker]
execute store result storage sgp.ci:summon_multiple callback_context.untouched int 1 if entity @e[tag=sgp.test.summon_callback,scores={sgp.dummy=0},distance=..2,type=marker]
execute store success storage sgp.ci:summon_multiple callback_context.caller_changed byte 1 if entity @e[tag=sgp.test.summon_callback_source,scores={sgp.dummy=1..},distance=..2,type=marker]
...
assert data storage sgp.ci:summon_multiple callback_context{called:3,untouched:1,caller_changed:0b}
data remove storage sgp.ci:summon_multiple callback_context

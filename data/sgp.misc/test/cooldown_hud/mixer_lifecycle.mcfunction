...
#
# The real Mixer registers the player, replaces and expires rewards, and runs the SGP HUD render override.
...
assert score #ci.mixer.segments sgp.dummy matches 1

# Poison render outputs so a missing or shadowed SGP render override cannot pass.
data remove storage sgp:actionbar_hud overlay
data remove storage dah:actbar parsing
...
# The cooldown HUD must still render when Mixer has no normal actionbar segments.
data remove storage sgp:actionbar_hud overlay
function dah.actbar_mixer:z_private/display/prepare
function sgp.ci:cooldown_hud/expect {kit:"archer",frame:20}

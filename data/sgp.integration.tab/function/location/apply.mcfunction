#> sgp.integration.tab:location/apply
# `{lieu: string}`

$execute if score @a[tag=sgp.tab_target,limit=1] sgp.tab_applied matches 1 if entity @a[tag=sgp.tab_target,tag=sgp.tab.location.$(lieu),limit=1] run return run function sgp.integration.tab:location/no_change

function sgp.misc:loop_as_entity/init {list_location:"sgp:data markers_lists.location", command:"run function sgp.integration.tab:location/remove_applied_tag with entity @s data"}

...
execute as @a[tag=in_game] store result score @s sgp.killer on attacker run data get entity @s UUID.[1]
...
execute if entity @p[scores={sgp.death_effect=1..}] as @a[tag=in_game] if score @s sgp.UUID = @p[scores={sgp.death_effect=1..}] sgp.killer run function sgp.cosmetics:kill_effects/summon
scoreboard players set @a[tag=in_game] sgp.death_effect 0

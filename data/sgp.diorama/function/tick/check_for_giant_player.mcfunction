#> sgp.diorama:tick/check_for_giant_player
...
$execute positioned ~-4 ~-4 ~-4 as @a[dx=$(mdx_end),dy=$(mdy_end),dz=$(mdz_end),tag=!sgp.inside_current_model] \
    run tag @s add sgp.around_current_model

tag @a[tag=sgp.around_current_model] add sgp.around_model

$execute as @a[tag=sgp.has_giant_mannequin_$(id),tag=!sgp.around_current_model] \
    run function sgp.diorama:tick/update_mannequin/disappear {type:"giant", id:$(id)}
...
$execute as @a[tag=sgp.around_current_model,tag=!sgp.has_giant_mannequin_$(id)] \
    at @e[predicate=bs.link:link_equal,tag=sgp.marker,name=playable_map,limit=1,type=marker] \
        run function sgp.diorama:tick/on_player_around_model {id:$(id)}
...
data modify storage sgp.ci:locations player_isolation set value {}
...
execute store result storage sgp.ci:locations player_isolation.first_found int 1 run scoreboard players get @s sgp.lieu_count
execute store result storage sgp.ci:locations player_isolation.first_inside int 1 run scoreboard players get @s sgp.ab.location
execute store result storage sgp.ci:locations player_isolation.other_found_before int 1 run scoreboard players get LocationOther sgp.lieu_count
execute store result storage sgp.ci:locations player_isolation.other_inside_before int 1 run scoreboard players get LocationOther sgp.ab.location

tp @s ~8.5 ~1 ~1.5
tp LocationOther ~1.5 ~1 ~1.5
function sgp.ci:locations/scan {tag:"sgp.test.location_players"}
execute store result storage sgp.ci:locations player_isolation.other_found_after int 1 run scoreboard players get LocationOther sgp.lieu_count
execute store result storage sgp.ci:locations player_isolation.other_inside_after int 1 run scoreboard players get LocationOther sgp.ab.location
dummy LocationOther leave
kill @e[tag=sgp.test.location_players,distance=..32,type=marker]

assert data storage sgp.ci:locations player_isolation{first_found:1,first_inside:1,other_found_before:0,other_inside_before:0,other_found_after:1,other_inside_after:1}

#> sgp.world:lieu/scan_excluded/1
# `{dx, dy, dz, lieu, lieu_propre, couleur, width, exclusion_box}`: the registry snapshot of one marker.
# Shard 1 (generated): same logic as lieu/lieu_trouve for a marker with an exclusion box; the
# box comes from the snapshot and lieu/main/1 is this shard's private copy of lieu/main.
...
$execute as @a[dx=$(dx), dy=$(dy), dz=$(dz)] \
    unless function sgp.world:lieu/check_exclusion_macro \
        run function sgp.world:lieu/main/1 {lieu_propre:"$(lieu_propre)", couleur:"$(couleur)", lieu:"$(lieu)", width:$(width)}
$execute as @a[dx=$(dx), dy=$(dy), dz=$(dz)] \

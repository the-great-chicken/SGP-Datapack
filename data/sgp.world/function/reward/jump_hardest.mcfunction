...
execute as @s[scores={sgp.jump_hardest_done=2}] \
    run tellraw @s [{"storage":"sgp.text", "nbt":"prefix", "interpret":true}, {"translate":"sgp.world:unlocked_marine_particle", "fallback":"Tu viens de débloquer la Trainée de Particule %s !\n        Va dans la Salle des %s depuis la %s pour l'activer !", "color":"aqua", "with":[{"translate":"sgp.world:particle_marine", "fallback":"Marine", "bold":true, "color":"blue"}, {"translate":"sgp.world:cosmetics", "fallback":"Cosmétiques", "color":"light_purple", "bold":true}, {"translate":"sgp.world:main_lobby", "fallback":"Salle d'Accueil"}]}]

...
execute if score #pigeons_secondes sgp.timer matches 60 run tellraw @a[tag=sgp.in_game] [{"text":"Il reste ","bold":true,"color":"blue"},{"score":{"name":"#invasion_minutes","objective":"sgp.timer"},"color":"gold"}," minute(s) !"]
...
execute if score #pigeons_minutes sgp.timer matches 10 if score #chasseurs_tues sgp.dummy >= #pigeons_joueurs sgp.dummy run tellraw @a[tag=sgp.in_game] [{"text":"Les Pigeons ont gagné !","color":"gray","bold":true}]
...
execute if score #pigeons_minutes sgp.timer matches 10 if score #chasseurs_tues sgp.dummy < #pigeons_joueurs sgp.dummy run tellraw @a[tag=sgp.in_game] [{"text":"Les Pigeons n'ont pas atteint le nombre de kills minimum, ÉGALITÉ","color":"yellow","bold":true}]

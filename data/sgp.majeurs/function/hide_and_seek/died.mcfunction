...
execute unless entity @a[team=sgp.hider] run title @a[tag=sgp.in_game] title [{text:"Les Chasseurs gagnent !", color:red}]
execute unless entity @a[team=sgp.hider] run tellraw @a[tag=sgp.in_game] [{storage:"sgp.text", nbt:"prefix", interpret:true}, {text:"Les ", color:gold}, {text:"Chasseurs ", color:dark_green}, {text:"ont gagné ! Ils ont tué toute la",color: gold}, {text:" Volaille", color:yellow}]
execute unless entity @a[team=sgp.hider] run return run function sgp.majeurs:hide_and_seek/_stop

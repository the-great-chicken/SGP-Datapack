...
$execute at @e[type=marker,tag=sgp.marker,name="devenir_roi_$(side)",limit=1] as @a[distance=..2,tag=sgp.major_participant] run team join sgp.$(team) @s
...
$execute at @e[type=marker,tag=sgp.marker,name="devenir_roi_$(side)",limit=1] as @a[distance=..2,tag=sgp.major_participant] run function sgp.kits:clear_and_tp_to_kits

$scoreboard players set @s sgp.devenir_roi_$(side) 0

$execute as @a[tag=sgp.major_participant] run trigger sgp.devenir_roi_$(side) set 0

...
execute if score #kings_chosen sgp.dummy matches 1 \
    unless entity @a[predicate=sgp.majeurs:protect/roi_rouge_vivant] \
        run function sgp.majeurs:protect/if_king_dead {team:rouge, name:Rouge,name_ennemies:Bleu, color:dark_red, color_ennemies:dark_blue}

execute unless entity @a[team=sgp.rouge] run return 1

execute if score #kings_chosen sgp.dummy matches 1 \
    unless entity @a[predicate=sgp.majeurs:protect/roi_bleu_vivant] \
        run function sgp.majeurs:protect/if_king_dead {team:bleue, name:Bleu, name_ennemies:Rouge, color:dark_blue, color_ennemies:dark_red}
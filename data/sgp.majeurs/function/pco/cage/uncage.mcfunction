...
$execute as @e[type=marker,tag=sgp.marker,tag=sgp.enabled,name="pco_uncage_storage",nbt={data:{cage:$(cage)}}] \
...
$function #bs.schedule:schedule {run:"execute as @e[type=marker,tag=sgp.marker,tag=sgp.enabled,name='pco_cage_storage',nbt={data:{cage:$(cage)}}] \
    run function sgp.majeurs:pco/cage/clone_cage with entity @s data",with:{time:3,unit:"s"}}
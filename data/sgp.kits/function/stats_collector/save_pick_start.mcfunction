...
# `{player_id, kit_id, pick_time: <ticks>, paused_ticks: <ticks>}`
...
$data modify storage sgp.kits:stats kits_dict.$(player_id).$(kit_id).pick.paused_ticks set value $(paused_ticks)

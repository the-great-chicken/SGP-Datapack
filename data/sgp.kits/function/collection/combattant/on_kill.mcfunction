#> sgp.kits:collection/combattant/on_kill
# 
# Gives the Combattant kill rewards

execute if score @s sgp.kills_give_1 matches 1.. run function sgp.kits:kills_give/basic { \
    nb:1, threshold:1, \
    give:"arrow 3", \
    give_2:air, \
    actionbar:{text:"+ 3 ➶ Flèches !", color:gray, bold:true}, \
    width:179, \
    }

execute if score @s sgp.kills_give_2 matches 2.. run function sgp.kits:kills_give/basic { \
    nb:2, threshold:2, \
    give:'golden_apple[ \
        custom_name={text:"Pomme d\'or", color:white, italic:false, bold:true}, \
        lore=[ \
             [ \
                {text:"Régénère jusqu\'à 6", color:gray, italic:false}, \
                {text:"❤", color:red}, \
                {text:" + 2"}, \
                {text:"❤", color:yellow} \
                ] \
            ] \
        ]', \
    give_2:air, \
    actionbar:{text:"+ 1 ❤ Pomme d'or !", color:yellow, bold:true}, \
    width:222, \
    }

# Consume every earned payout, retaining progress toward the next one.
execute if score @s sgp.kills_give_1 matches 1.. run return run function sgp.kits:collection/combattant/on_kill
execute if score @s sgp.kills_give_2 matches 2.. run return run function sgp.kits:collection/combattant/on_kill

...
$execute positioned ~ ~0.6 ~ rotated $(rotation) 0 run function #bs.raycast:run {with:{max_distance:12, entities:"!sgp.radiator", on_hit_point:"execute unless score $raycast.hit_flag bs.lambda matches -1 run scoreboard players operation #ray_dist_$(direction) sgp.dummy = $raycast.distance bs.lambda", piercing: {entities: 50}, on_targeted_entity:"tag @s[tag=!sgp.peaceful] add sgp.to_damage"}}

# Remember to replace @e by @a
execute as @e[tag=sgp.to_damage] run damage @s 0.1 sgp.kits:ray by @p[tag=sgp.radiator]
execute as @e[tag=sgp.to_damage] at @s run particle lava ~ ~0.6 ~ 0 0 0 0 1
tag @e[tag=sgp.to_damage] remove sgp.to_damage

...
dummy UnlockAgainWatch spawn
tag UnlockAgainWatch add sgp.in_game
function sgp.kits:unlocking/unlocking_kit {kit:"tank",kit_color:"gray",fw_color:8421504}
assert score @s sgp.tank_found matches 1
assert chat ".*déjà trouvé ce kit.*" @s
assert not chat ".*a trouvé le kit tank.*" UnlockAgainWatch
assert not chat ".*déjà trouvé ce kit.*" UnlockAgainWatch

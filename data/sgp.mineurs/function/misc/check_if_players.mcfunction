...
execute if score #nbr_de_joueurs sgp.dummy matches ..1 run tellraw @a[tag=sgp.in_game] [{"storage":"sgp.text", "nbt":"prefix", "interpret":true}, {"translate":"sgp.mineurs:not_enough_players", "fallback":"Il n'y a pas suffisamment de personnes dans l'arène...", "color":"dark_red"}]

...
execute as @s[scores={sgp.jump_diff_2_done=2}] run tellraw @s ["",{"text":"Vous venez de débloquer le Kill Effect "},{"text":"Splash","bold":true, "color": "blue"},{"text":" !\nAllez dans la Salle des ", "color": "white"},{"text":"Cosmétiques","color":"light_purple","bold":true},{"text":" depuis la Salle d'Accueil pour l'activer !"}]
execute as @s[scores={sgp.jump_diff_2_done=2}] run scoreboard players set @s splash_kill_unlocked 1

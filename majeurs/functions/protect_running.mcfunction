...
execute unless predicate majeurs:roi_rouge_vivant if score #mort_roi_rouge_annoncee test matches 0 run tellraw @a [{"text":"Le roi de l'Equipe ", "color":"gold"},{"text":"Rouge ", "color":"dark_red", "bold":true},{"text":"est vivant, les membres de son équipe ne peuvent plus réapparaitre!", "color":"gold"}]
execute unless predicate majeurs:roi_rouge_vivant if score #mort_roi_rouge_annoncee test matches 0 run tp @a[x=2423,y=251.5,z=2145.5,distance=..3,team=!rouge,team=!bleue] 2531 251.5 2198 135 0
execute unless predicate majeurs:roi_rouge_vivant if score #mort_roi_rouge_annoncee test matches 0 run tag @a remove roi_rouge
execute unless predicate majeurs:roi_rouge_vivant if score #mort_roi_rouge_annoncee test matches 0 run scoreboard players set #mort_roi_rouge_annoncee test 1
...
execute unless predicate majeurs:roi_bleu_vivant if score #mort_roi_bleu_annoncee test matches 0 run tellraw @a [{"text":"Le roi de l'Equipe ", "color":"gold"},{"text":"Bleue ", "color":"dark_blue", "bold":true},{"text":"est vivant, les membres de son équipe ne peuvent plus réapparaitre!", "color":"gold"}]
execute unless predicate majeurs:roi_bleu_vivant if score #mort_roi_bleu_annoncee test matches 0 run tag @a remove roi_bleu
execute unless predicate majeurs:roi_bleu_vivant if score #mort_roi_bleu_annoncee test matches 0 run scoreboard players set #mort_roi_bleu_annoncee test 1
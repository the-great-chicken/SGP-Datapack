...
# Timer Écoulé
execute if score #invasion_secondes timer >= #invasion nbr_de_joueurs run tellraw @a[x=2405,y=201,z=2133,dx=137,dy=54,dz=72] [{"text":"Les Défenseurs ont gagné ! Ils ont survécu suffisament longtemps","color":"blue","bold":true}]
execute if score #invasion_secondes timer >= #invasion nbr_de_joueurs run title @a[x=2405,y=201,z=2133,dx=137,dy=54,dz=72] title [{"text":"Défenseurs Vainqueurs","color":"blue","bold":true}]
execute if score #invasion_secondes timer >= #invasion nbr_de_joueurs run function majeurs:invasion_stop

# Prévenir qu'il reste peu de temps
execute if score #invasion_ticks timer matches 0 run scoreboard players operation #invasion-100 timer = #invasion nbr_de_joueurs

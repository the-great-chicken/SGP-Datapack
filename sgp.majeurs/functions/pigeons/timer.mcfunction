...
execute if score #pigeons_secondes sgp.timer matches 60 run tellraw @a [{"text":"Il reste ","bold":true,"color":"blue"},{"score":{"name":"#invasion_minutes","objective":"sgp.timer"},"color":"gold"},{"text":" minute(s) !","color":"blue"}]
...
execute if score #pigeons_minutes sgp.timer matches 10 if score #chasseurs_tues sgp.dummy >= #pigeons_joueurs sgp.dummy run tellraw @a [{"text":"Les sgp.Pigeons ont gagné !","color":"gray","bold":true}]
execute if score #pigeons_minutes sgp.timer matches 10 if score #chasseurs_tues sgp.dummy >= #pigeons_joueurs sgp.dummy run title @a title [{"text":"Les sgp.Pigeons ont gagné !","bold":true,"color":"gray"}]

# Si insuffisament de kills de chasseurs
execute if score #pigeons_minutes sgp.timer matches 10 if score #chasseurs_tues sgp.dummy < #pigeons_joueurs sgp.dummy run tellraw @a [{"text":"Les sgp.Pigeons n'ont pas atteint le nombre de kills minimum, ÉGALITÉ","color":"yellow","bold":true}]
